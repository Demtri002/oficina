        ...
        DO $$ BEGIN
             IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'fk_servico_3') THEN
                ALTER TABLE servico ADD CONSTRAINT FK_servico_3
                    FOREIGN KEY (fk_tipo_servico_id_tiposervico) REFERENCES tipo_servico (id_tiposervico) ON DELETE CASCADE;
             END IF;
        END $$;
        """,
        """
...
def cadastrar_tipo_servico(conn, nome, descricao):
    sql = """
        INSERT INTO tipo_servico (nome, descricao)
        VALUES (%s, %s)
    ...
    try:
        with conn.cursor() as cur:
            ...
            id_gerado = cur.fetchone()[0]
        conn.commit()
        print(f"✅ Tipo de Serviço '{nome}' cadastrado. ID: {id_gerado}")
        return id_gerado
    except DatabaseError as e:
        conn.rollback()
        print(f"❌ Erro ao cadastrar tipo de serviço: {e}")
        return None
...
def cadastrar_faturamento(conn, valor_total):
    sql = """
        INSERT INTO Faturamento (valor_total)
        VALUES (%s)
        RETURNING id_faturamento;
    """
    try:
        with conn.cursor() as cur:
            cur.execute(sql, (valor_total,))
            id_gerado = cur.fetchone()[0]
        conn.commit()
        print(f"✅ Faturamento registrado. ID: {id_gerado}")
        return id_gerado
    except DatabaseError as e:
        conn.rollback()
        print(f"❌ Erro ao registrar faturamento: {e}")
        return None
...
def cadastrar_servico(conn, nome, descricao, id_veiculo, id_tipo_servico):
    sql = """
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(sql, (nome, descricao, id_veiculo, id_tipo_servico))
            id_gerado = cur.fetchone()[0]
        conn.commit()
